...
uniform float zDist;    // New uniform for z-distance threshold
uniform float zCone;    // New uniform for z-cone factor
...
void main () {
    ...
    if (pos2d.z < zDist - zCone * max(abs(pos2d.y), abs(pos2d.x)) || pos2d.x < -clip || pos2d.x > clip || pos2d.y < -clip || pos2d.y > clip) {
...
let defaultViewMatrix = [
  0.97, -0.05, -0.24, 0, -0.03, 0.96, -0.29, 0, 0.25, 0.29, 0.92, 0, -0.75,
  -0.74, 8.99, 1,
];
...
async function main() {
  ...
  const u_zDist = gl.getUniformLocation(program, "zDist");
  const u_zCone = gl.getUniformLocation(program, "zCone");

  // Initialize uniform values
  let viewDistance = 10.0;
  gl.uniform1f(u_zDist, viewDistance);
  gl.uniform1f(u_zCone, 0.9);

  // Initialize orbit distance to match z-distance
  window.orbitDistance = viewDistance;
  ...
  window.addEventListener(
    ...
    (e) => {
      ...
      if (e.shiftKey) {
        ...
        // Scale the camera position when zooming
        const zoomFactor = 1 + (-10 * (e.deltaY * scale)) / innerHeight;
        inv[12] *= zoomFactor;
        inv[13] *= zoomFactor;
        inv[14] *= zoomFactor;

        // Update the view distance
        window.viewDistance *= zoomFactor;
        window.orbitDistance = window.viewDistance;

        // Update the distance slider and display
        const distanceSlider = document.getElementById("distance");
        const distanceValue = document.getElementById("distanceValue");
        if (distanceSlider && distanceValue) {
          distanceSlider.value = window.viewDistance;
          distanceValue.textContent = window.viewDistance.toFixed(1);
        }

        // Update the shader uniform
        gl.uniform1f(u_zDist, window.viewDistance);
      } else {
        inv = translate4(inv, 0, 0, window.orbitDistance);
  ...
  const frame = (now) => {
    ...
    if (carousel) {
      ...
      // Calculate time-based rotation angle (slower than before)
      const angle = -0.0003 * (Date.now() - start);

      // Apply orbit rotation around vertical axis only
      inv = translate4(inv, 0, 0, window.orbitDistance);
  ...
  // Move the controls creation and event listener setup into a function
  function initializeControls(gl, program) {
    ...
    controls.innerHTML = `
      <div style="color: white; margin-bottom: 10px;">
        <label>View Distance: <span id="distanceValue">8.0</span></label>
        <input type="range" id="distance" min="1" max="20" step="0.1" value="8.0" style="width: 200px; display: block">
      </div>
      <div style="color: white; margin-bottom: 10px;">
        <label>Z-Cone: <span id="zConeValue">0.4</span></label>
        <input type="range" id="zCone" min="0" max="2" step="0.1" value="0.4" style="width: 200px; display: block">
      </div>
    `;
    ...
    // Initialize values
    window.viewDistance = 8.0; // Make it globally accessible
    window.orbitDistance = window.viewDistance; // Make orbit distance match view distance

    // Set initial uniform values
    gl.uniform1f(u_zDist, window.viewDistance);
    gl.uniform1f(u_zCone, 0.4);

    // Add event listeners after the elements are created
    const distanceSlider = document.getElementById("distance");
    const zConeSlider = document.getElementById("zCone");

    distanceSlider.addEventListener("input", (e) => {
      const newDistance = parseFloat(e.target.value);
      const oldDistance = window.viewDistance;

      // Update the view distance values
      window.viewDistance = newDistance;
      window.orbitDistance = newDistance;

      // Update display and uniform
      document.getElementById("distanceValue").textContent =
        newDistance.toFixed(1);
      gl.uniform1f(u_zDist, newDistance);

      // Adjust camera position to maintain orbit center
      let inv = invert4(viewMatrix);
      const scaleFactor = newDistance / oldDistance;

      // Scale the translation component to maintain the same view direction
      inv[12] *= scaleFactor;
      inv[13] *= scaleFactor;
      inv[14] *= scaleFactor;
    ...
    zConeSlider.addEventListener("input", (e) => {
      const value = parseFloat(e.target.value);
      document.getElementById("zConeValue").textContent = value.toFixed(1);
      gl.uniform1f(u_zCone, value);
    });
  }

  // In your main function, just call initializeControls
  initializeControls(gl, program);
}
