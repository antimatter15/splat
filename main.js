...
void main() {
    ...
    // 2) Fetch the splat’s world position
    uvec4 cen = texelFetch(
        u_texture,
        ivec2((uint(index) & 0x3ffu) << 1, uint(index) >> 10),
        0
    );
    ...
    uvec4 cov = texelFetch(
        u_texture,
        ivec2(((uint(index) & 0x3ffu) << 1) | 1u, uint(index) >> 10),
        0
    );

    // Extract color from covariance data
    vec3 shColor = vec3((cov.w) & 0xffu, (cov.w >> 8) & 0xffu, (cov.w >> 16) & 0xffu) / 255.0;
    ...
    float radius  = length(vec2((cov2d[0][0] - cov2d[1][1]) * 0.5,
                                cov2d[0][1]));
    ...
    // 5) Output the color & quad position
    float alpha = float((cov.w >> 24) & 0xffu) / 255.0;
    vColor    = vec4(shColor, alpha);
    vPosition = position;
